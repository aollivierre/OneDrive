...
# Use the production scripts with embedded logging
# These scripts have the entire logging module embedded within them
$detectionScript = Join-Path $scriptPath "Detect-OneDriveConfiguration-RMM-Production.ps1"
$remediationScript = Join-Path $scriptPath "Remediate-OneDriveConfiguration-RMM-Production.ps1"
...
# Run the actual script in a subprocess to prevent exit from terminating wrapper
`$exitCode = 0
try {
    if (`$global:EnableDebug) {
        ...
        # Run in subprocess to capture exit code without terminating wrapper
        `$process = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", "& '$ScriptToTest' -EnableDebug; exit `$LASTEXITCODE" -Wait -PassThru -NoNewWindow
        ...
        # Run in subprocess to capture exit code without terminating wrapper
        `$process = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", '$ScriptToTest' -Wait -PassThru -NoNewWindow
        `$exitCode = `$process.ExitCode
    }
